...
char objectMenuEntries[numMeshes][128] = {
  ...
  "56 Ginger Bread Man", "57 Monkey Head", "59 Second Ginger Bread Man"
};
...
//drag callbacks set to do nothing first
static void doNothingCallback(vec2 xy) { return; }

//pointers to a function that does nothing
...
static void doToolUpdateXY(int x, int y) { 
	...
	//if left button
        if(currButton==GLUT_LEFT_BUTTON)
			//gets how far mouse has moved and multiples by the left button
			//trans matrix?
...
//sets button call back to new call back function and updates button translation matrix
...
static void activateTool(int button) {
    //updates current button
    currButton = button;
    //updates coordinate of last mouse click (relative to window)
    clickPrev = currMouseXYscreen(mouseX, mouseY);
    //updates previous position with last click position
	prevPos = clickPrev;
